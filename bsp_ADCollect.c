...
void TDS_Calulate(void)
{
	...
	if(sTDS_Calulate < 30)//TDS每隔500ms检测一次,前面3秒钟AD数据不计算
	{
			sTDS_Calulate++;
			if(sTDS_Calulate < 25)
			{
...
void test_TDS(u16 D, float *pp)
{
		...
		else if(gstADCollect.fChunShui <= 2220)
		{
				gstADCollect.tds_ChunShui  = adc_to_tds(gstADCollect.fChunShui,2000,-704);//300
		}

		else if(gstADCollect.fChunShui <= 2600)
		{
				gstADCollect.tds_ChunShui  = adc_to_tds(gstADCollect.fChunShui,3000,-1200);//400
		}


		else if(gstADCollect.fChunShui <= 2800)
		{
				gstADCollect.tds_ChunShui  = adc_to_tds(gstADCollect.fChunShui,6000,-2000);//500
		}

		else if(gstADCollect.fChunShui <= 3000)
		{
				gstADCollect.tds_ChunShui  = adc_to_tds(gstADCollect.fChunShui,12000,-3500;//600
		}

		else 
		{
				gstADCollect.tds_ChunShui  = adc_to_tds(gstADCollect.fChunShui,24000,-6500);//700
		}
	#else

		if(v <= 753)//20
		{
				*p = adc_to_tds(v,172.4,-3.4);//30
		}
		
		else if(v<= 880)
		{
				*p  = adc_to_tds(v,200,-6);//40
		}
		else if(v <= 1175)
		{
				*p   = adc_to_tds(v.5,-9.5);//50
		}
		
		else if(v= 1415)
		{
				*p  = adc_to_tds(v,421.7,-57.9);//85
		}
		...
		{
				*p   = adc_to_tds(v,1500,-478);//200
		}
		

		else if(v <= 2250)
		{z
				*p   = adc_to_tds(v,2000,-704);//300
		}

		else if(v <= 2600)
		{
				*p   = adc_to_tds(v,3000,-1200);//400
		}


		else if(v<= 2900)
		{
				*p  = adc_to_tds(v,6000,-2000);//500
		}

		else if(v <= 3300)
		{
				*p   = adc_to_tds(v,12000,-3500;//600
		}

		else 
		{
				*p = adc_to_tds(v,24000,-6500);//700
		}
	}
