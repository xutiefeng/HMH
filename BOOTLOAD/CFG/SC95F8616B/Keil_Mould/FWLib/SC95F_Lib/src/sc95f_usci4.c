...
StatusTypeDef USCI4_SPI_Receive(USCI4_HandleInfoDef* USCI4_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  if(USCI4_HANDLE->RxState == USCI4_STATE_READY)
  {
    ...
    {
      return USCI4_SPI_TransmitReceive(USCI4_HANDLE, pData, pData, Size, Timeout);			//跳转到阻塞模式数据收发函数
    }
...
/**********************************************************************************************************
*函数名称:StatusTypeDef USCI4_SPI_TransmitReceive(USCI4_HandleInfoDef* USCI4_HANDLE, uint8_t* pTxData, uint8_t* pRxData, uint8_t Size, uint32_t Timeout)
*函数功能:阻塞模式收发一定量数据
*入口参数:
USCI4_HandleInfoDef*:USCI4_HANDLE:指向包含 SPI0 信息的 USCI4_HandleInfoDef 结构体的指针
...
StatusTypeDef USCI4_SPI_TransmitReceive(USCI4_HandleInfoDef* USCI4_HANDLE, uint8_t* pTxData, uint8_t* pRxData, uint8_t Size, uint32_t Timeout)
{
  ...
  if((USCI4_HANDLE->TxState == USCI4_STATE_READY) && (USCI4_HANDLE->RxState == USCI4_STATE_READY))
  {
    ...
    USCI4_ClearFlag(USCI4_SPI_FLAG_SPIF);			//清除中断标志位
    USCI4_HANDLE->pTxBuffPtr.Size_u8 = pTxData;       //指向待发送数据的地址
    ...
    while(USCI4_HANDLE->TxXferCount < USCI4_HANDLE->TxXferSize);
    {
      ...
      if((US0CON1 & USCI4_SPI_DATA16) != 0)
      {
        ...
        while(!USCI4_GetFlagStatus(USCI4_SPI_FLAG_SPIF))			//等待发送完成
        {
          ...
          if(TimeoutCnt++ > Timeout)
          {
            USCI4_HANDLE->RxState = USCI4_STATE_TIMEOUT;//接收错误
      ...
      TimeoutCnt = 0;			//超时计数值清零
      USCI4_HANDLE->TxXferCount++;			//数据量计数值加1
      USCI4_HANDLE->TxXferCount++;			//数据量计数值加1
    }
    if(USCI4_HANDLE->TxXferCount == USCI4_HANDLE->TxXferSize)
    {
