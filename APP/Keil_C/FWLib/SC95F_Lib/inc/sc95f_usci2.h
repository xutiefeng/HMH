...
typedef enum
{
  ...
  USCI2_SPI_FLAG_WCOL = (uint8_t)0x40, //SPI写入冲突标志位WCOL
...
typedef enum
{
  USCI2_STATE_READY = 0x00,   //准备标志
  USCI2_STATE_BUSY = 0x01,    //忙碌标志
  USCI2_STATE_ERROR = 0x02,   //错误标志
  USCI2_STATE_TIMEOUT = 0x03,  //超时标志
	USCI2_STATE_WAIT = 0x04,   //等待标志
} USCI2_StatusTypeDef;

typedef struct __USCI2_HandleInfoDef
{
  union
  {
  ...
  USCI2_StatusTypeDef TxState;      //发送状态
  USCI2_StatusTypeDef RxState;      //接收状态
} USCI2_HandleInfoDef;
...
StatusTypeDef USCI2_TWI_Slave_Transmit(USCI2_HandleInfoDef* USCI2_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout);
