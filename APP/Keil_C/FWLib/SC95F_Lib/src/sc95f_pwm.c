...
*函数功能:ErrorStatus PWM_ComplementaryMode_DutyChange(PWM_ComplementaryOutputPin_TypeDef PWM_ComplementaryOutputPin,
    PWM_DutyChange_TypeDef Change_Direction, uint16_t DutyIncremental)
*入口参数:
...
/*****************************************************
*函数名称:ErrorStatus PWM_ComplementaryMode_DutyChange(PWM_ComplementaryOutputPin_TypeDef PWM_ComplementaryOutputPin,
    PWM_DutyChange_TypeDef Change_Direction, uint16_t DutyIncremental)
*函数功能:PWM互补模式占空比修改
*入口参数:
PWM_ComplementaryOutputPin_TypeDef:PWM_ComplementaryOutputPin:待修改的PWM互补通道
                                 ...
                                 uint16_t PWM_DutyCycle)
{
  ...
  PWMREG[PWM40 + PWM_ComplementaryOutputPin * 4] = (PWMREG[PWM40 + PWM_ComplementaryOutputPin * 2] & 0xF0) | (PWM_DutyCycle / 256);
}
...
/*****************************************************
*函数名称:ErrorStatus PWM_IndependentMode_DutyChange(PWM_OutputPin_TypeDef PWM_OutputPin, PWM_DutyChange_TypeDef Change_Direction, uint16_t DutyIncremental)
*函数功能:PWM独立模式占空比修改
*入口参数:
PWM_OutputPin_TypeDef:PWM_OutputPin:待修改的PWM独立通道
PWM_DutyChange_TypeDef:Change_Direction:PWM修改方向
    ...
    PWM_DutyChange_TypeDef Change_Direction, uint16_t DutyIncremental)
{
  ...
  tmpRegValue = ((PWMREG[PWM_OutputPin] << 8) | PWMREG[PWM_OutputPin + 1]);			//把当前占空比寄存器放进临时变量
  if(Change_Direction == PWM_DutyChange_Up)
  {
    tmpRegValue += DutyIncremental;				//占空比增加
  }
    ...
    PWM_DutyChange_TypeDef Change_Direction, uint16_t DutyIncremental)
{
  ...
  tmpRegValue = ((PWMREG[PWM40 + PWM_ComplementaryOutputPin * 4] << 8) | PWMREG[PWM40 + PWM_ComplementaryOutputPin * 4 + 1]);			//把当前占空比寄存器放进临时变量
  ...
  else /* PWM2/3/4相关寄存器复位到保留值 */
  {
    ...
    switch(PWM_Type)
    {
      ...
      case PWM3_Type:
        ...
        (uint32_t)PWMREG[2] = 0;
        break;
#if !defined(SC95F7610B) && !defined(SC95F7619B) && !defined(SC95R751)
      case PWM4_Type:
        IE1 &= ~0x80;
        IP1 &= ~0x80;
        (uint32_t)PWMREG[4] = 0;
...
*出口参数:void
**************************************************/
void PWM_OutputStateConfig(PWM_OutputPin_TypeDef PWM_OutputPin,
                           PWM_OutputState_TypeDef PWM_OutputState)
{
...
*出口参数:void
**************************************************/
void PWM_PolarityConfig(PWM_OutputPin_TypeDef PWM_OutputPin,
                        PWM_Polarity_TypeDef PWM_Polarity)
{
  unsigned char Reg_Data;
  if((PWM_OutputPin & 0xF0) == 0x00)
...
/*****************************************************
*函数名称:ErrorStatus PWM_ComplementaryMode_DutyChange(PWM_ComplementaryOutputPin_TypeDef PWM_ComplementaryOutputPin, PWM_DutyChange_TypeDef Change_Direction, uint16_t DutyIncremental)
*函数功能:PWM互补模式占空比修改
*入口参数:
PWM_ComplementaryOutputPin_TypeDef:PWM_ComplementaryOutputPin:待修改的PWM互补通道
PWM_DutyChange_TypeDef:Change_Direction:PWM修改方向
