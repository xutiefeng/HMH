...
typedef enum
{
  ...
  USCI3_SPI_FLAG_WCOL = (uint8_t)0x40, //SPI写入冲突标志位WCOL
...
typedef enum
{
  USCI3_STATE_READY = 0x00,   //准备标志
  USCI3_STATE_BUSY = 0x01,    //忙碌标志
  USCI3_STATE_ERROR = 0x02,   //错误标志
  USCI3_STATE_TIMEOUT = 0x03,  //超时标志
  USCI3_STATE_WAIT = 0x04,   //等待标志
} USCI3_StatusTypeDef;

typedef struct __USCI3_HandleInfoDef
{
  union
  {
  ...
  USCI3_StatusTypeDef TxState;      //发送状态
  USCI3_StatusTypeDef RxState;      //接收状态
} USCI3_HandleInfoDef;
typedef enum
{
